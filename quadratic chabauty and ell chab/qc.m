  ...
  // ===                   CHECK INPUT                      ===
  // ==========================================================
SetLogFile("qcexample.log");

_<x> := PolynomialRing(Rationals());
f := x^6 + (18/5)*x^4 + (6/5)*x^3 + (9/5)*x^2 + (6/5)*x + (1/5);
X_aff := HyperellipticCurve(f);
...
X, phi := Transformation(X_aff, [0,1,1,0]);
...
qc_primes, groups, good_primes, prime_quality :=
       find_qc_primes(H : mws_primes_bound := 6000, qc_primes_bound := 300, number_of_bad_disks := 0, inf_modp_allowed := false, known_rat_pts := ptsH);

//quality is inversely related to actual quality of life

p := 53;      // prime number p
rho := 2;
N := 20; prec :=40; basis0 := []; basis1 := []; basis2 := [];
printlevel := 2; debug := false; base_point := [-3, 1];
    hecke_prime := 0; unit_root_splitting := true; eqsplit := 0;
  height_coeffs := []; rho := 0; use_log_basis := false;
Q := y^2 - f;
  ...
  factors_away_known_rat_points_3 := [];
  for i->pt in good_Qpoints do
    if i ne global_base_point_index then
      if GF(3)!lindepQp(pt`x) eq 0 then //(new)
          Append(~factors_away_known_rat_points_3,Rationals()!0);
          continue;
      end if;
      if GF(3)!lindepQp(pt`x) eq 1 then //x/z = 1 mod 3 goes to z/x = 1 mod 3 //old and new
        Append(~factors_away_known_rat_points_3,Rationals()!8/3);
        continue;
      end if;
      print pt;
      assert false;
...
factors_away_known_rat_points_5 := [];
for i->pt in good_Qpoints do
  if i ne global_base_point_index then
    if GF(5)!(lindepQp(pt`x)) eq 0 then
      Append(~factors_away_known_rat_points_5,-3);
      continue;
    end if;
    if GF(5)!lindepQp(pt`x) eq 3 then  
      Append(~factors_away_known_rat_points_5,3);
      continue;
    else
      Append(~factors_away_known_rat_points_5,0);
    end if;
  end if;
end for;

  all_factors_away_3 := [0, 8/3];
  all_factors_away_5 := [0, 3, -3];
  c:= 1; //p is 53,17
  //c:=-2;
  //c := 1/2; //p is 19
  //c:= -1;//p = 7
  away_contributions_3 := [c*a*Log(Qp!3) : a in all_factors_away_3];
  away_contributions_5 := [c*a*Log(Qp!5) : a in all_factors_away_5];

  away_contributions := [a + b : a in away_contributions_3, b in away_contributions_5];

  factors_away_known_rat_points_3_log := [c*a*Log(Qp!3) : a in factors_away_known_rat_points_3];
  factors_away_known_rat_points_5_log := [c*a*Log(Qp!5) : a in factors_away_known_rat_points_5];
    ...
    if #height_coeffs eq 0 or not use_log_basis then // Compute heights of auxiliary points.
      ...
      if pl gt 1 then
        printf " Using point %o at correspondence %o to generate.\n",
                                  good_affine_rat_pts_xy_no_bpt[min_i],1;
      end if;
      ...
      if #heights lt g then  // add E1_E2(P) to known subspace until dimension is g.
        ...
        repeat
          ...
          if Dimension(new_E1_E2_subspace) gt Dimension(E1_E2_subspace) then
            ...
            print "with height", factors_away_known_rat_points_3[i]+factors_away_known_rat_points_5[i] ;
            print "this is point index ", i;
            gammafilP := evalf0(ChangeRing(gammafil,LaurentSeriesRing(BaseRing(gammafil))),Qpti,data);
            height_P := height(Phii,betafil,gammafilP,eqsplit,data)+factors_away_known_rat_points_3_log[i]+factors_away_known_rat_points_5_log[i];
  ...
  // Find expansion of the quadratic Chabauty function
    ...
    for l := 1 to numberofpoints do
      if G_list[l] eq 0 then
        ...
        global_height := &+[height_coeffs[j,1]*Eltseq(E1_E2_lists[1,l])[j] : j in [1..g]];
        F_list[l] := global_height - local_height_lists[1,l];
      end if;
    ...
    c2 := Min([0, valbetafils[1], minvaleqsplit, valbetafils[1]+ minvaleqsplit]);

    i0 := 0;
    i0_threshold := Min([valetas[1], valbetafils[1]/2, (minvalgammafils[1]-c2)/2]);
    repeat
      i0 +:= 1;
    ...
    function valF(i)
      ...
      assert i ge i0;
      valgammafili := i le maxdeggammafils[1] select minvalgammafils[1] else 0;
      return -2*Floor(log(p,i)) +c1s[1] + Min(c2,c3+2*minvalchangebasis);
    end function;

    zero_list := [* *];
    sol_list  := [* *];

    Nend := Integers()!Min(Nexpansions[1], Nhtcoeffs); // Precision used for root finding
...
K := Universe(fake_rat_pts[1]);
X_p := ChangeRing(X, K);
X_inf_p, phi_p := Transformation(X_p, [0,1,1,0]);
psi_p := Inverse(phi_p);

Xpts := [X!P : P in good_affine_rat_pts_xy];
good_pts3 := [(X_p!P) : P in good_affine_rat_pts_xy cat bad_affine_rat_pts_xy];
for P in good_pts3 do
  Include(~Xpts, P);
end for; 
// check that we're not missing any small points
_, rho := IsIsomorphic(H, X);
small_pts := Points(H : Bound := 1000); 
small_pts := [rho(P) : P in small_pts];
...
//Translate to H, because it's better to work without denominators

divisors := [* [* [[1,-15]],[[-2,15]]*],    [* [[-2,-15]], [[0,-5]] *]*];
P1:=H![1,-15]-H![-2,15];
P2 := H![-2,-15]-H![0,-5];
splitting_generators := [P1,P2];
splitting_indices := [[1,0], [0,1]];


"Use independent points", splitting_generators;
f:= HyperellipticPolynomials(H);
Q := y^2 - f;
data_hol := coleman_data(Q,p,10:useU:=false);

fake_rat_pts_H := [ [A[1],A[2]] where A is (rho^(-1))(fake_rat_pts[i]) : i in [1..#fake_rat_pts] ];

good_affine_rat_pts_xy_H := [[A[1],A[2]] where A is (rho^(-1))(X!good_affine_rat_pts_xy[i]) : i in [1 .. #good_affine_rat_pts_xy]];

fake_rat_pts_small :=[];
for i in [1..#fake_rat_pts] do
  fake_rat_pts_small[i] := [ChangePrecision(fake_rat_pts_H[i,j], 10) : j in [1..2]];
  // lower precision for speed and to avoid issues in Coleman integrals.
end for;
fake_coeffs_mod_pN, rat_coeffs_mod_pN := coefficients_mod_pN(fake_rat_pts_small, good_affine_rat_pts_xy_H, divisors, base_pt_H, splitting_indices, data_hol); 
...
"generating cosets";
time qc_fake_coeffs_mod_M := coeffs_CRT(fake_coeffs, [p], [3]);
"number of cosets", #qc_fake_coeffs_mod_M;
qc_M := p^3;  // modulus

mws_primes := sieving_primes(qc_M, good_primes, groups, 10); // compute sieving primes
//[ 97, 151, 157, 271, 281, 421, 487, 971, 1877 ]

time done_fake,bool_list, satknown, satunknown := MWSieve(J, mws_primes, qc_M, bas, base_pt_H, qc_fake_coeffs_mod_M : known_rat_coeffs := rat_coeffs_mod_pN);
"done with the Mordell-Weil sieve?", done_fake;
"For sieving used the prime(s)", mws_primes;
bad_indices := [i : i in [1 .. #bool_list] | bool_list[i] eq true];

print "still deal with", bad_indices;
print "the points";
 badpts := [fake_rat_pts_H[bad_indices[i]] : i in [1..#bad_indices]];
print badpts;

//for p =53

print "also have dealt with bad points", bad_affine_rat_pts_xy;

print "points at infinity are empty?", PointsAtInfinity(ChangeRing(X, GF(p)));

modulus := Exponent(AbelianGroup(BaseChange(J, GF(p))));

bad_pts_p := [[18, 44,1],[18,9,1]]; 

coeffs_mod_Mp := prerun_mwsieve_g2r2(J, bas, base_pt_H, modulus, p, bad_pts_p);
mws_primes_p := sieving_primes(modulus, good_primes, groups, 10);  // compute sieving primes

time done_fake,bool_list, satknown, satunknown := MWSieve(J, mws_primes_p, modulus, bas, base_pt_H, coeffs_mod_Mp);

"now done with Mordell-Weil sieve?", done_fake;
